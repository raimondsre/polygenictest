process VCF_PGS_post_processing {
    ...
    echo "library(dplyr); library(data.table); sscore <- fread("plink_score_file") %>% mutate(sampleset = "new_file") %>% select(sampleset,IID=`#IID`,SUM=PGS001296_hmPOS_GRCh38_SUM); fread('homogenised_file') %>% select(sampleset,IID,SUM) %>% cbind(sscore) %>% mutate(percentile_sum = ntile(SUM, 100)) %>% filter(sampleset != 'reference') %>% mutate(percentile_sum_local = ntile(percentile_sum,100)) %>% fwrite('percentile_calculated.txt',sep='\\t')" > to_execute.R
    Rscript to_execute.R
    # Rscript -e "library(dplyr); library(data.table);a <- fread('homogenised_file'); a %>% select(sampleset,IID,SUM)  %>% mutate(percentile_sum = ntile(SUM, 100)) %>% filter(sampleset != 'reference') %>% mutate(percentile_sum_local = ntile(percentile_sum,100)) %>% fwrite('percentile_calculated.txt',sep='\\t')"
