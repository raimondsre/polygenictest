# raimondsre/lvbmc-polygenictest: Usage
...
```bash
nextflow run lvbmc/polygenictest -profile docker -params-file params.yaml
```
...
```bash
nextflow pull raimondsre/lvbmc-polygenictest
```
...
First, go to the [raimondsre/lvbmc-polygenictest releases page](https://github.com/raimondsre/lvbmc-polygenictest/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
